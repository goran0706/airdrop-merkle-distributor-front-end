...
const ClaimButton = styled.button`
	...
	:hover {
		box-shadow: ${({ disabled }) =>
			disabled ? '0px 4px 16px rgba(0, 0, 0, 0.7)' : '0 0 2px #000'};
...
const AirdropDetails = () => {
	...
	useEffect(() => {
		const updateState = async () => {
			...
			if (claimAccounts.includes(account?.toLowerCase())) {
				const { index, proof, amount } =
					claimAccountsArr[claimAccounts.indexOf(account.toLowerCase())]
				setIndex(index)
				setProof(proof)
				setAmount(amount)
	...
	return (
		<AirdropWrapper>
			...
			{!isClaimed && amount && (
				<Paragraph>
					This account has not yet claimed the tokens. Total of {amount} tokens are
