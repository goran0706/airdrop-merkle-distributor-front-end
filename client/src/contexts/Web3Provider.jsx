...
function Web3Provider({ children }) {
	...
	const switchNetwork = async chainId => {
		try {
			await rpcRequest('wallet_switchEthereumChain', [{ chainId: toHex(chainId) }])
			setNetwork(await library.getNetwork())
			setChainId(toHex(chainId))
		} catch (error) {
			if (error.code === 4902) {
				try {
					await rpcRequest('wallet_addEthereumChain', [{ chainId: toHex(chainId) }])
				} catch (error) {
					setError(error)
				}
			}
	...
	const verifyMessage = async () => {
		...
		try {
			const verify = await rpcRequest('personal_ecRecover', [signedMessage, signature])
